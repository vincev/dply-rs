...
The following is a list of the functions that are currently supported with
examples, more examples can be found in the [tests folder](tests).
...
See [tests](tests/group_by.rs) for more examples.
...
[tests](tests/mutate.rs) for more examples.
...
move columns before or after a specific column, see [tests](tests/relocate.rs)
for examples.
